...
if($_SESSION['ISLOGIN']){
	if($_SESSION['USERAUTHORITY']){
		$query=mysql_query($_SESSION['sql-file_stay']);
		...
		while ($row = mysql_fetch_array($query)) {
			...
			$objExcel->getActiveSheet()->setCellValue('f'.$u1, $row["file_content"]);
			$objExcel->getActiveSheet()->setCellValue('g'.$u1, $row["reason"]);
			$objExcel->getActiveSheet()->setCellValue('h'.$u1, $row["object"]);
			$objExcel->getActiveSheet()->setCellValue('i'.$u1, $row["phone_num"]);
			$objExcel->getActiveSheet()->setCellValue('j'.$u1, $row["gra_year"]);
			$i++;
		}
		...
		header('Content-Disposition: attachment;filename="FileStayData'.$timestamp.'.xls"');
