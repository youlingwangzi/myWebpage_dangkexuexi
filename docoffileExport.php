...
if($_SESSION['ISLOGIN']){
	if($_SESSION['USERAUTHORITY']){
		$query=mysql_query($_SESSION['sql-docoffile']);
		...
		while ($row = mysql_fetch_array($query)) {
			...
			$u1=$i+2;
			$objExcel->getActiveSheet()->setCellValue('a'.$u1, $row["title"]);
			$objExcel->getActiveSheet()->setCellValue('b'.$u1, $row["author"]);
			$objExcel->getActiveSheet()->setCellValue('c'.$u1, $row["date"]);
			$objExcel->getActiveSheet()->setCellValue('d'.$u1, $row["content"]);
			$objExcel->getActiveSheet()->setCellValue('e'.$u1, $row["path"]);
			$i++;
		}
