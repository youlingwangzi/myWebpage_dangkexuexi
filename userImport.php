...
mysql_select_db($dbdatabase, $db);
 if (! empty ( $_FILES ['file_stu'] ['name'] )) {
	...
	for($j=2;$j<=$highestRow;$j++){//从第一行开始读取数据
		...
		//explode:函数把字符串分割为数组。
		$strs=explode("\\",$str);
		$sql="INSERT INTO logins(username, password, isSuperAdmin, real_name, department, phone_number, e_mail) VALUES (
		'".$strs[0]."',
		'".$strs[1]."',
		0,
		'".$strs[2]."',
		...
		?>
		<tr> 
            <td class = "username">		<span><?php echo $strs[0] ?></span> </td>
			<td class = "real_name">	<span><?php echo $strs[2] ?></span> </td>
			<td class = "department">	<span><?php echo $strs[3] ?></span> </td>
			<td class = "phone_number">	<span><?php echo $strs[4] ?></span> </td>
			<td class = "e_mail">	<span><?php echo $strs[5] ?></span> </td>
			<td class = "isSuperAdmin">	<span><?php echo "用户"?></span>
        </tr> 
		<?php
	}
	?>
...
?>
<h1>导入Excel表：</h3>
<form method="post" action="mainAdmin.php?fid=1&mid=6" enctype="multipart/form-data">
